...
colormap2 = [
    [0.0, "rgb(85, 156, 178)"],
    [0.20, "rgb(245, 238, 248)"],
    [0.40, "rgb(244, 236, 247)"],
    [0.60, "rgb(253, 237, 236)"],
    [0.80, "rgb(245, 183, 177)"],
    [1.0, "rgb(195, 115, 99)"],
]
    ...
    def get_scatterplot_fig(self):
        ...
        trace1 = go.Scattergl(
            ...
            marker=dict(
                ...
                line=dict(color="#F5B7B1", width=self.score_tag),
            ),
            name="embeddings ",
        )
...
app.layout = html.Div(
    ...
    children=[
        html.H1(
            ...
            style={"textAlign": "center", "color": "gray", "font-family": "monospace"},
        ),
        dcc.RadioItems(
            id="radio-items",
            style={"textAlign": "center", "color": "gray", "font-family": "monospace"},
        ...
        html.Div(
            [
                dcc.Graph(id="tsne-graph", figure=fig_returned2),
                dcc.Graph(id="attention-graph", figure=fig_returned),
            ],
        ...
        html.Div(
            [
                ...
                html.Button(
                    id="toggle-button",
                    n_clicks=0,
                    children="Click here to toggle affiliation",
                ),
                html.Pre(id="toggle-data", style=styles["pre"]),
                html.Button(
                    id="save-button", n_clicks=0, children="Export current state"
                ),
...
@app.callback(Output("click-data", "children"), [Input("tsne-graph", "clickData")])
def display_click_data(clickData):
    ...
    if clickData:
        ...
        c_classification = (
            "MISMATCH"
            if (elem.get("marker.line.width") == 5)
            else "Matches corpus affiliation"
        )
        ...
        return_text = (
            ...
            + c_text
            + "\n\n"
            + "Affiliation in corpus: "
            + c_label
            + "\n\n"
            + "Classification affiliation: "
            + c_classification
            + "\n\n"
            + "Normalized classification strength: "
            + str(c_classification_strength)
            + "\n\n"
            + "Tweet ID: "
            + str(c_tweet_id)
        )
...
@app.callback(Output("attention-graph", "figure"), [Input("tsne-graph", "clickData")])
...
@app.callback(Output("toggle-data", "children"), [Input("toggle-button", "n_clicks")])
def display_click_data(n_clicks):
    ...
    if selected_point:
        ...
        print(
            "Affiliation in training file",
            tsne_obj.df.ix[int(selected_point)].text,
            tsne_obj.df.ix[int(selected_point)].label,
        )
        tsne_obj.df.ix[int(selected_point), "label"] = int(
            abs(1 - tsne_obj.df.ix[int(selected_point)].label)
        )
...
@app.callback(Output("save-data", "children"), [Input("save-button", "n_clicks")])
def save_data_callback(n_clicks):
    global tsne_obj
    print(tsne_obj.df.columns)
    tsne_obj.df.to_csv(
        header=False, columns=["text", "label"], path_or_buf="Exported.csv"
    )
...
@app.callback(Output("tsne-graph", "figure"), [Input("radio-items", "value")])
