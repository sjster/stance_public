...
class IteratorClass():
    ...
    def __init__(self):
        ...
        self.batch_parameters = {'batch_size':128, 'WRAP_AROUND':False, 'RUN_TILL_END_RESTART':False, 'RUN_TILL_END_AND_STOP':True}
    ...
    def get_train_iterator(self, batch_parameters):
        return(self.get_batch( self.train_ids, self.train_text_list, self.train_labels, batch_parameters ))


    def get_valid_iterator(self, batch_parameters):
        return(self.get_batch( self.valid_ids, self.valid_text_list, self.valid_labels, batch_parameters ))


    def get_test_iterator(self, batch_parameters):
        return(self.get_batch( self.test_ids, self.test_text_list, self.test_labels, batch_parameters ))
    ...
    def get_batch(self, ids , text, labels, batch_parameters):
        ...
        while( curr < total_size ):
            ...
            if((end < start) and batch_parameters['WRAP_AROUND'] == True):
                ...
                elem = current_batch(data_return, torch.tensor(label_return), text_return)

                yield(elem)

            elif((end < start) and (( batch_parameters['RUN_TILL_END_RESTART'] == True) or (batch_parameters['RUN_TILL_END_AND_STOP'] == True))):

                print("Getting to the end")
                DONE_FLAG = True
                elem = current_batch(ids[curr:], torch.tensor(labels[curr:]), text[curr:])
                yield(elem)

            else:

                elem = current_batch(ids[start: end], torch.tensor(labels[start: end]), text[start: end])
                yield(elem)
...
class RNN(nn.Module):
    ...
    def forward(self, text, batch_lengths=[]):
        ...
        if(DEBUG == True):
            print("Output of forward shape before squeeze",hidden.shape, hidden.squeeze(0).shape)
...
class ModelClass(VisClass):
    ...
    def get_data(self):
        ...
        self.train_iterator, self.valid_iterator, self.test_iterator, self.vocab = self.data_iterator.train_iterator, \
self.data_iterator.valid_iterator, self.data_iterator.test_iterator, self.data_iterator.TEXT.vocab


    def get_iterators(self):

        self.data_iterator.get_iterators()
        self.train_iterator, self.valid_iterator, self.test_iterator = self.data_iterator.train_iterator, self.data_iterator.valid_iterator, self.data_iterator.test_iterator
    ...
    def train_model(self):
        ...
        self.model = RNN(INPUT_DIM, EMBEDDING_DIM, HIDDEN_DIM, OUTPUT_DIM, N_LAYERS, BIDIRECTIONAL, DROPOUT)
    ...
    def predict(self):
            ...
            with torch.no_grad():
                ...
                for ct,batch in enumerate(self.test_iterator):
                    ...
                    if(PACKED_FLAG):
                        ...
                        predictions_array.extend(list(zip( sentences, label.cpu().tolist(), score.squeeze().tolist(), penultimate_layer.tolist(), attention_weights.cpu().detach().numpy() )))
            ...
            self.df = pd.DataFrame(predictions_array, columns = ['text', 'label', 'score', 'penultimate','attention_weights'])
