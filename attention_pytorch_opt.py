...
def func(HIDDEN_DIM_IN, DROPOUT_IN, LINEAR_DIM_IN):
        ...
        def plotly_2d_scattergl_embeddings_single(self, X_reduced, filename, title):
            ...
            trace1 = go.Scattergl(
                ...
                marker=dict(
                    ...
                    size=15,
                    colorscale="Jet",
                    line=dict(color="#EBFBEC", width=self.entity_ring_size),
                ),
                name="embeddings ",
            )
    ...
    class IteratorClass:
        def __init__(self, device, DEBUG):
            ...
            self.SEQ_LEN = data.Field(
                dtype=torch.int64, use_vocab=False, sequential=False
            )
            ...
            self.tst = TabularDataset(
                path=DATA_FOLDER
                + "/Test.csv",  # the root directory where the data lies
                format="csv",
                skip_header=False,  # if your csv header has a header, make sure to pass this to ensure it doesn't get proceesed as data!
                fields=self.tv_datafields,
            )
        ...
        def build_vocabulary(self):
            ...
            self.train_iterator = data.BucketIterator(
                self.trn, batch_size=self.BATCH_SIZE, device=self.device
            )

            self.valid_iterator = data.BucketIterator(
                self.vld, batch_size=self.BATCH_SIZE, device=self.device
            )
    ...
    def get_batch_lengths(data, label, record_id=None):
        ...
        return_seq_len = [
            seq_len[sorted_index] for sorted_index in seq_len_sorted_index
        ]
    ...
    class ModelClass(VisClass):
        ...
        def predict(self):
            ...
            self.entity_transparency, self.entity_ring_size = get_score(
                self.df["score"], self.df["label"]
            )
            self.vis(
        ...
        def get_normalized_weights(weight_list):
            ...
            return_list = list(
                map(lambda x: (x[0], x[1] / total_weight), filtered_list)
            )
            # print(return_list)
            return return_list
...
for experiment in opt.get_experiments():
    ...
    # Test the model
    loss, acc = func(
        experiment.get_parameter("HIDDEN_DIM"),
